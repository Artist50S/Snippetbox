...
func main() {
	...
	//Определение нового флага из командной строки для настройки mysql connect
	...
	//пула соединений в отделььную функцию openDB(). мы передаем в нее полученный
	//источник данных (DSN) из флага командной строки.
	db, err := openDB(*dsn)
	if err != nil {
		errorLog.Fatal(err)
	}
	//мы так же откладываем вызов db.Close(), чтобы пул соединений
	//был закрыт до выхода из функции main()
	defer db.Close()
	...
	infoLog.Println("start web-servis on %s", *addr)
...
// функия openDB() обертывает sql.Open()  и возвращае пул соединений
// для заданой строки подключения (DSN)
